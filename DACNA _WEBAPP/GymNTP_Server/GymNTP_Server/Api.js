...
// Sign-in route
app.post('/api/post/signin', async (req, res) => {
  const { phoneNumber, password } = req.body;
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query('SELECT * FROM Employee WHERE PhoneNumber = @PhoneNumber AND Password = @Password');
...
// CRUD cho bảng Bills
// Create a new bill
app.post('/api/post/bills', async (req, res) => {
  try {
...
// Read 
app.get('/get/latestbillidbycustomer/:id', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const result = await pool.request()
      .input('CustomerID', sql.Int, id)
      .query('SELECT MAX(BillID) AS LatestBillID FROM Bills WHERE CustomerID = @CustomerID');

    // Check if there's a result
    if (result.recordset.length > 0) {
      const latestBillID = result.recordset[0].LatestBillID;
      res.json({ latestBillID });
    } else {
      res.status(404).send({ message: 'Không tìm thấy BillID cho CustomerID đã cho' });
    }
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
...
app.post('/api/post/attendance', async (req, res) => {
  try {
    ...
    await pool.request()
      ...
      .query('INSERT INTO Attendance (BillID, CheckInTime) VALUES (@BillID, @CheckInTime)');
    res.status(201).send({ message: 'Attendance created successfully' });
  } catch (err) {
    res.status(500).send({ message: err.message });
  }
});
